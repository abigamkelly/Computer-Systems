...
int main(int argc, char **argv) 
{
    ...
    while(1) {
        ...
        /* if the user enters '1', prompt the user to enter the
           first name, last name, age, and major */
        if (strcmp(input, "1") == 0) {
            strcat(buf, input);         /* choice to buffer */
            ...
            fgets(input, MAXLINE, stdin);
            strtok(input, "\n");
            strcat(buf, input);         /* add age to buffer */
            strcat(buf, ",");           /* add ',' to buffer */
            printf("Enter major: ");
            fgets(input, MAXLINE, stdin);
            strcat(buf, input);         /* add major to buffer */
            ...
            printf("Invalid option. Please select option 1, 2, or 3.\n");
            continue;
        }
